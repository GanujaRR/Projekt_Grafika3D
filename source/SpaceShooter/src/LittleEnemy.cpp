...
void LittleEnemy::OnInit(glm::vec3 position, float rotation, glm::vec3 size)
{
	...
	bar = new HealthBar();
	bar->OnInit(glm::vec3(1000, 1000, 0 ), 0, glm::vec3(80, 5, 0));
...
void LittleEnemy::OnRender(SpriteRenderer* renderer)
{
	if (isAlive)
	{
		renderer->Draw(Texture::GetTexture("littleEnemy"), Shader::GetShader("sprite"), position, rotation, scale, glm::vec4(1));
