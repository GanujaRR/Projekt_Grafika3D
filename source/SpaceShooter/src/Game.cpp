...
void Game::Init()
{
	...
	// Texture Init
	Texture::LoadTexture("space", "Texture/pixel_space.png");
	Texture::LoadTexture("bullet", "Texture/pink_bullet.png");
	Texture::LoadTexture("bg", "Texture/background-black.png");
	Texture::LoadTexture("littleEnemy", "Texture/litteEnemy.png");
	Texture::LoadTexture("heart", "Texture/heart.png");
	Texture::LoadTexture("boss", "Texture/boss.png");
	Texture::LoadTexture("win", "Texture/win.png");
	Texture::LoadTexture("lose", "Texture/lose.png");
	...
	showLose->OnInit(glm::vec3(0, 600, 0), 0, glm::vec3(400));
	InitBulletPool();
}
...
void Game::OnUpdate(float ds)
{
	...
	if (space->IsOver() == true) {
		canShoot = false;
		showLose->ShowLoseUI(space, boss, enemySpawnManager);
	}
	showLose->OnUpdate(ds);
...
void Game::OnRender()
{
	...
	showLose->OnRender(renderer);

	// player render
	space->OnRender(renderer);
}

// Tworzenie vectora pocisków
...
void Game::DoCollision()
{
	if (space->IsAlive())
	{
		...
		for (int i = 0; i < enemySpawnManager->GetEnemies().size(); i++)
		{
			if (!enemySpawnManager->GetEnemies()[i]->IsAlive()) // if enemy is not alive then do nothing !
				continue; 
			bool collisionX = abs(space->GetPosition().x - enemySpawnManager->GetEnemies()[i]->GetPosition().x) <=
				space->GetSize().x / 2.f - 10.f + enemySpawnManager->GetEnemies()[i]->GetSize().x / 2.f - 10.f;
			bool collisionY = abs(space->GetPosition().y - enemySpawnManager->GetEnemies()[i]->GetPosition().y) <=
				space->GetSize().y / 2.f - 15.f + enemySpawnManager->GetEnemies()[i]->GetSize().y / 2.f - 15.f;
			
			if (collisionX && collisionY)
			{
				space->TakeDamage(1);
				enemySpawnManager->GetEnemies()[i]->Destroy();
			}
		}
		...
		for (int i = 0; i < player_bulletPool.size(); i++)
		{
			...
			for (int j = 0; j < enemySpawnManager->GetEnemies().size(); j++)
			{
				if (!enemySpawnManager->GetEnemies()[j]->IsAlive()) // If enemy is not alive then do nothing !
					continue;

				bool collisionX = abs(player_bulletPool[i]->GetPosition().x - enemySpawnManager->GetEnemies()[j]->GetPosition().x) <=
					player_bulletPool[i]->GetSize().x / 2.f - 10 + enemySpawnManager->GetEnemies()[j]->GetSize().x / 2.f - 10;

				bool collisionY = abs(player_bulletPool[i]->GetPosition().y - enemySpawnManager->GetEnemies()[j]->GetPosition().y) <=
					player_bulletPool[i]->GetSize().y / 2.f - 15 + enemySpawnManager->GetEnemies()[j]->GetSize().y / 2.f - 15;
					
				if (collisionX && collisionY)
				{
					enemySpawnManager->GetEnemies()[j]->TakeDamage(40);
